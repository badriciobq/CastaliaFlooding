...
void BypassMAC::fromNetworkLayer(cPacket * pkt, int destination)
{
	...
	trace() << "MAC - Node[" << SELF_MAC_ADDRESS << "] sendind data to: " << destination ; 
...
{
	trace() << "MAC - Node[" << SELF_MAC_ADDRESS << "] receiving data."; 
	    ...
	    macPkt->getDestination() == BROADCAST_MAC_ADDRESS)
	{
		...
		trace() << "MAC - Node[" << SELF_MAC_ADDRESS << "] receiving data to: " << macPkt->getDestination(); 
	}
}
